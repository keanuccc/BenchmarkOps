...
- 选择所属项目、选择文件（支持 **JSONL / JSON / CSV / TSV / XLSX**，扩展名自动识别格式）、填写名称与描述。大文件会在**后台异步导入**并轮询行级进度，失败时展示行级错误；0 行空文件会被服务端拒绝。
- **格式**：每行一个 JSON 对象，至少含输入与期望输出字段，例如：
  ```json
  {"question": "Compute 2 + 2.", "answer": "4"}
  ```
- 上传限制：**单文件 ≤ 50 MB，行数 ≤ 100,000 行**（超过将被拒绝，见 §6）。
- **多轮 / 少样本**：上传时勾选"结构化对话"（`structured_chat=true`）后，行数据可携带 `messages`（对话历史）与 `examples`（少样本示例）字段，运行器会自动组装消息链；未勾选时行为与普通文本评测完全一致。
