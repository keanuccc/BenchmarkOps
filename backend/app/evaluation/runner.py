...
async def run_experiment(experiment_id: str) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        row_repo = DatasetRowRepository(session)
        ...
        try:
            ...
            if snap:
                ...
                prompt = await session.get(Prompt, experiment.prompt_id)
                if prompt is None:
                    raise ValueError("Experiment references a missing prompt")
            ...
            if bsnap:
                ...
                benchmark = await session.get(Benchmark, experiment.benchmark_id)
                if benchmark is None:
                    raise ValueError("Experiment references a missing benchmark")
            ...
            if msnap:
                ...
                model = await session.get(Model, experiment.model_id)
                if model is None:
                    raise ValueError("Experiment references a missing model")
                model_ref = model.model_id
                pricing = model.pricing

            if not metric_name:
                raise ValueError("Experiment has no scoring metric")
            ...
            # Clean out any prior results (retry safety).
            ...
            # Cursor-based paging: fetch _BATCH_SIZE rows at a time so the whole
            # dataset never has to materialize in memory at once.
            offset = 0
            while True:
                ...
                for row in batch:
                    ...
                    try:
                        ...
                        score = float(
                            metric_fn(
                                completion.text, expected_str, **(metric_config or {})
                            )
                        )
                        cost = _cost(
                            pricing or {},
                    ...
                    if processed % _PROGRESS_EVERY == 0:
                        # Persist progress on a separate session so the UI can poll
                        # it without waiting for the run to finish.
                        await exp_repo.update(
                            experiment,
                            {"progress": processed, "rows_total": offset + len(batch)},
                        )
            ...
            fresh = await exp_repo.get(experiment_id)
            if fresh is not None:
                await exp_repo.update(fresh, {"status": "failed", "error": str(exc)[:500]})
                await session.commit()
