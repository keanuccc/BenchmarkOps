...
class Dataset(Base, UUIDMixin, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            "format IN ('csv', 'tsv', 'json', 'jsonl', 'xlsx')",
...
class DatasetVersion(Base, UUIDMixin, TimestampMixin):
    """Immutable snapshot of dataset content/metadata at a given version."""
...
class DatasetRow(Base, UUIDMixin, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "dataset_id", "version", "idx",
            name="uq_dataset_rows_dataset_version_idx",
        ),
    )
